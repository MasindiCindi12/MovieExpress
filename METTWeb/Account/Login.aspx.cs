      ...
      this.RedirectLocation = VirtualPathUtility.ToAbsolute(Security.GetSafeRedirectUrl(Page.Request.QueryString["ReturnUrl"], "~/default.aspx"));
		...
		public Result Login(LoginDetails loginDetails)
		{
			...
			try
			{
				...
				if (MEIdentity.Login(loginDetails).Success == true)
				{//MEIdentity.Login(loginDetails);
					ret.Success = true;
					bool User = MEWebSecurity.CurrentIdentity().FirstTimeLogin;

					if (User == true)
					{
						if (MEWebSecurity.CurrentIdentity().FirstTimeLogin)
						{
							ret.Data = "FirstTimeDeposit.aspx";
							// Masindi First Time Login come here
						}
						else
						{
							ret.Success = true;
                ...
                {
					ret.ErrorText = "User Not Registered.";
					ret.Success = false;
				}
            }
			catch
